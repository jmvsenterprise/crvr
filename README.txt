...
This server serves any file from the directory its launched in. So to launch
your server navigate to your web root directory and run the server binary. It
does, however, have a special page: asl.html, which redirects to an embedded
"application", which expects to find the asl.html and asl_done.html. Eventually
I want to extract these into a loadable shard object file in the future, but
my primary goal was to get this working for me. So stay tuned for that in the
future.
...
FUTURE ENHANCEMENTS
-------------------
Save card data when a quiz is completed. For each card, save the player's score
